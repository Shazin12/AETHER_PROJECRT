...
void main() {
  group('Aether Raid Concurrency Integrity', () {
    ...
    setUp(() async {
      ...
      try {
        raidService = RaidService(fakeFirestore);
      } catch (e) {
        fail(
      ...
      await fakeFirestore.collection('events').doc('dragon_raid').set(
        <String, dynamic>{'joinedCount': 0, 'capacity': 15},
      );
    });
    ...
    test(
      ...
      () async {
        final List<Future<RaidJoinResult>> joinRequests =
        ...
        final List<RaidJoinResult> results = await Future.wait(joinRequests);
        final int successfulJoins = results
            .where((RaidJoinResult result) => result.success == true)
            .length;

        final DocumentSnapshot<Map<String, dynamic>> snapshot =
            await fakeFirestore.collection('events').doc('dragon_raid').get();
        final int slotsFilled = snapshot.data()?['joinedCount'] ?? 0;
